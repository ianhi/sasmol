...
class Files(object):
    ...
    def read_single_dcd_step(self, filename, frame):
        ...
        for i in xrange(frame):
            result = dcdio.read_dcdstep(
                infile, tx, ty, tz, num_fixed, first, reverseEndian, charmm)
    ...
    def read_dcd_step(self, dcdfile, frame, **kwargs):
        ...
        result = dcdio.read_dcdstep(
            filepointer, tx, ty, tz, num_fixed, frame, reverseEndian, charmm)

        self._coor[0, :, 0] = tx.astype(numpy.float)
        self._coor[0, :, 1] = ty.astype(numpy.float)
        self._coor[0, :, 2] = tz.astype(numpy.float)
    ...
    def read_dcd(self, filename):
        ...
        for i in xrange(nset):
            ...
            result = dcdio.read_dcdstep(
                infile, tx, ty, tz, num_fixed, i, reverseEndian, charmm)
...
#
    ...
    def write_pdb(self, filename, frame, flag, **kwargs):
        ...
        for i in range(len(self._atom)):
            ...
            try:
                ...
                infile.write("%-6s%5s %-4s%1s%-4s%1s%4s%1s   %8s%8s%8s%6s%6s      %-4s%2s%2s\n" % (self._atom[i], this_index, self._name[i], self._loc[i], self._resname[
                             i], self._chain[i], this_resid, self._rescode[i], sx, sy, sz, self._occupancy[i], self._beta[i], self._segname[i], self._element[i], self._charge[i]))
            except:
                print '\n>>>> ERROR IN WRITE_PDB <<<<\n'
                print '>> i = ', i
    ...
    def get_resnames(self):
        ...
        protein_resnames = ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS', 'HSD',
                            'HSE', 'HSP', 'ILE', 'LEU', 'LYS', 'MET', 'PHE', 'PRO', 'SER', 'THR', 'TRP', 'TYR', 'VAL']

        dna_resnames = ['NUSA', 'NUSG', 'NUSC', 'NUSU', 'DA',
                        'DG', 'DC', 'DT', 'ADE', 'GUA', 'CYT', 'THY']

        rna_resnames = ['RNUS', 'RNUA', 'RUUG', 'RNUC',
                        'A', 'C', 'G', 'U', 'ADE', 'CYT', 'GUA', 'URA']

        nucleic_resnames = ['GUA', 'ADE', 'CYT', 'THY', 'URA',
                            'G', 'A', 'C', 'T', 'U', 'DA', 'DG', 'DC', 'DT']
    ...
    def initialize_children(self):
        ...
        occupancies_mask = numpy.zeros(
            (number_of_occupancies, natoms), numpy.int)
